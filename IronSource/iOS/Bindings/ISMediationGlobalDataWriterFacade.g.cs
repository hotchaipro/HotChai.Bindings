	...
	public unsafe partial class ISMediationGlobalDataWriterFacade : NSObject {
		...
		public virtual void SetGoogleWaterMark (string waterMark)
		{
			if (waterMark is null)
				global::ObjCRuntime.ThrowHelper.ThrowArgumentNullException (nameof (waterMark));
			var nswaterMark = CFString.CreateNative (waterMark);
			if (IsDirectBinding) {
				ApiDefinitions.Messaging.void_objc_msgSend_NativeHandle (this.Handle, Selector.GetHandle ("setGoogleWaterMark:"), nswaterMark);
			} else {
				ApiDefinitions.Messaging.void_objc_msgSendSuper_NativeHandle (this.SuperHandle, Selector.GetHandle ("setGoogleWaterMark:"), nswaterMark);
			}
